...
The *generate_clusters* function should run in less than a minute, but your own larger datasets will take longer. The sample dataset has already been TPM and Z-score normalized. It is a 5000 gene x 150 sample data.frame. The 150 samples respresent 10 individuals (5 male, 5 female) each with 15 immunce cell types. The dataset was also filtered so that it would contain only protein coding genes. More information about this dataset can be found using the *?rnaSeq* command.
